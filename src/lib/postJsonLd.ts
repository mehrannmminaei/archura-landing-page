const DEFAULT_SITE_URL = 'https://www.archuramedia.com';
...
export function buildPostJsonLd(
  post: PostJsonLdInput,
  siteUrl = DEFAULT_SITE_URL,
): Record<string, unknown> {
  const base = siteUrl.replace(/\/$/, '');
  const canonical = post.seoCanonicalUrl || `${base}/blog/${post.slug}/`;
  const description = post.seoMetaDescription || post.excerpt || post.title;
  ...
  return {
    ...
    author: {
      ...
      url: `${base}/blog/author/${post.author.slug}/`,
    },
    publisher: {
      '@type': 'Organization',
      name: SITE_NAME,
